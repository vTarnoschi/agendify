...
const onboardingSchema = z.object({
  businessName: z.string().min(2, "O nome do negócio precisa ter pelo menos 2 caracteres."),
  slug: z.string().min(3, "O link personalizado precisa ter pelo menos 3 caracteres.")
    .regex(/^[a-z0-9-]+$/, "O link só pode conter letras minúsculas, números e hifens."),
});
...
export async function POST(req: Request) {
  try {
    ...
    if (existingSlug && existingSlug.clerkId !== clerkId) {
      return error("Este link já está em uso por outro profissional. Escolha outro.", 400);
    }
