...
const settingsSchema = z.object({
  ...
  workStart: z
    .string()
    .regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, "Horário de início inválido")
    .optional(),
  workEnd: z
    .string()
    .regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, "Horário de término inválido")
    .optional(),
...
export async function PUT(req: Request) {
  try {
    ...
    const { workingDays, workStart, workEnd, brandColor, brandLogo } =
      parsed.data;
